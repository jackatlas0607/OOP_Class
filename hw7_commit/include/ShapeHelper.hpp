...
public:
    bool IsBoxOverlap(BoxContainer boxContainer1, BoxContainer boxContainer2) {
        ...
        bool x_overlap = (x1 <= x2 && x2 <= x1 + boxContainer1.GetWidth()) ||
                         (x2 <= x1 && x1 <= x2 + boxContainer2.GetWidth());
        bool y_overlap = (y1 - boxContainer1.GetHeight() <= y2 && y2 <= y1) ||
                         (y2 - boxContainer2.GetHeight() <= y1 && y1 <= y2);
    ...
    bool IsPointInSquare(int sx, int sy, int edge, int tx, int ty) {
        ...
        bool insideY = (ty <= sy) && (ty >= sy_bottom);
